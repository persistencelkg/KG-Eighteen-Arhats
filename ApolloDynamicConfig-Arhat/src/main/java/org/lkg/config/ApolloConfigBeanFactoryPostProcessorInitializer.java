...
public class ApolloConfigBeanFactoryPostProcessorInitializer
    ...
    // initialize无论是主动@Import 导入还是 被动导入，都无法自定义生效，
    // 必须在spring-factories依托org.springframework.context.ApplicationContextInitializer启动
    @Override
...
//        });
    ...
    public void postProcessBeanFactory(ConfigurableListableBeanFactory beanFactory) throws BeansException {
        // internal config service
        DynamicConfigManger.registerConfigService(environment::getProperty);
        // 值过滤器，处理占位符的value
        DynamicConfigManger.addValueFilter(s -> {
            try {
        ...
        // 让动态配置中心早于 LongHengMeterRegistry的初始化，以便于能在一开始就能拿到初始化好的配置
